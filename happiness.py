...
with st.sidebar:
       selected = option_menu(menu_title = 'Navigation Pane',
              options = ['Abstract','Data Sets','Data Cleaning','Data Analysis','Conclusion'],
              menu_icon = 'arrow-down-right-square',
              icons = ['bookmark-check','bar-chart','pie-chart','check-circle'],
              default_index = 0)
...
if selected =='Data Sets':
       st.header('Data Sets')
       st.markdown('For this case study, I used 2 data sets and combined them into one.')
       st.markdown('This data set is the world happiness data of 2021')
       whr2021 = pd.read_csv("world-happiness-report-2021.csv")
       st.dataframe(whr2021)
       code12 = '''whr2021.describe()'''
       st.code(code12, language = 'python')
       code15 = whr2021.describe()
       st.write(code15)
       st.markdown('This data set is the world happiness data from 2005 to 2020.')
...
if selected == 'Data Analysis':
       ...
       happiness_revised.drop(['Positive affect','Negative affect'],axis = 1,inplace=True)
       merged_table = pd.concat([happiness_revised,world_happiness_21[list(happiness_revised.columns)+['Regional indicator']]])
       region_dict = {k:v for k,v in zip(world_happiness_21['Country name'],world_happiness_21['Regional indicator'])}
       merged_table['Region'] = merged_table['Country name'].replace(region_dict)
       merged_table.drop('Regional indicator',axis = 1,inplace = True)
